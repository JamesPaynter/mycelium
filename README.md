...
- Control plane: `control_plane.enabled` derives component resources (`component_resource_prefix`), uses `fallback_resource` for unmapped files, `scope_mode` controls compliance enforcement (off/shadow/enforce), `lock_mode` selects declared/shadow/derived scheduling, and `control_plane.checks.mode` (off/report/enforce) enables scoped doctor commands via `commands_by_component` with fallback to the global doctor.
