...
const CONTROL_PLANE_ACCEPTANCE_TIMEOUT_MS = 30000;
...
describe("control-plane acceptance", () => {
  ...
  it(
    "returns stable component and ownership results",
    async () => {
    ...
    CONTROL_PLANE_ACCEPTANCE_TIMEOUT_MS,
  );

  it(
    "returns stable dependency and blast outputs",
    async () => {
      const repoDir = await createTempRepoFromFixture();
      await buildControlPlaneModel({ repoRoot: repoDir });
  ...
  it(
    "returns stable TypeScript symbol results",
    async () => {
      const repoDir = await createTempRepoFromFixture();
      await buildControlPlaneModel({ repoRoot: repoDir });
