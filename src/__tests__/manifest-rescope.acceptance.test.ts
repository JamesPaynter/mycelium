...
describe("acceptance: manifest enforcement auto-rescopes and retries", () => {
  ...
  it(
    ...
    async () => {
      ...
      const updatedManifest = await readPlannedManifest(planResult.outputDir);
...
async function readPlannedManifest(outputDir: string): Promise<any> {
  ...
  const manifestPath = path.join(outputDir, relDir, "manifest.json");
